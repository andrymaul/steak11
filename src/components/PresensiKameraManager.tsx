...
export const PresensiKameraManager: React.FC<PresensiKameraManagerProps> = ({
  ...
  const handleDeleteRecord = (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data presensi ini?')) {
      ...
      if (showToast) showToast('Data rekap presensi berhasil dihapus!');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* REKAP PRESENSI DIGITAL & FILTER MULTI-KRITERIA SECTION */}
      {(() => {
        const filteredRecords = attendance.filter((rec) => {
          if (filterSearchName && !rec.employeeName.toLowerCase().includes(filterSearchName.toLowerCase())) {
            return false;
          }
          if (filterOutlet !== 'ALL' && rec.outlet !== filterOutlet) {
            return false;
          }
        ...
        const totalLateCount = filteredRecords.filter((r) => r.status === 'Terlambat' || r.clockInStatus === 'Terlambat Masuk').length;
        const totalOnTimeCount = filteredRecords.filter((r) => r.status === 'Hadir' && r.clockInStatus !== 'Terlambat Masuk').length;
        const totalEarlyOutCount = filteredRecords.filter((r) => r.clockOutStatus === 'Pulang Awal').length;
        ...
        return (
          <div className="bg-white dark:bg-[#1f0e30] p-6 rounded-2xl border border-slate-200 dark:border-purple-900/50 shadow-sm space-y-5">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-purple-900/50 pb-4">
              <div>
                <h3 className="font-extrabold text-lg text-[#3D1259] dark:text-amber-400 font-baloo flex items-center gap-2">
                  <Calendar className="w-5 h-5 text-amber-500" />
                  Rekap & Riwayat Presensi Digital Karyawan
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Filter rekap presensi berdasarkan rentang tanggal, nama karyawan, outlet, dan status shift secara real-time.
                </p>
              </div>

              {/* Summary Stats Badges */}
              <div className="flex flex-wrap items-center gap-2 text-xs">
                <span className="px-3 py-1.5 rounded-xl bg-purple-100 dark:bg-purple-950 text-purple-900 dark:text-amber-300 font-extrabold border border-purple-200 dark:border-purple-800">
                  📊 Total: <strong>{totalRecordsCount} Data</strong>
                </span>
                <span className="px-3 py-1.5 rounded-xl bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 font-extrabold border border-emerald-200 dark:border-emerald-800">
                  ✅ Hadir: <strong>{totalOnTimeCount}</strong>
                </span>
                <span className="px-3 py-1.5 rounded-xl bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 font-extrabold border border-amber-200 dark:border-amber-800">
                  ⚠️ Terlambat: <strong>{totalLateCount}</strong>
                </span>
                <span className="px-3 py-1.5 rounded-xl bg-rose-100 dark:bg-rose-950/80 text-rose-800 dark:text-rose-300 font-extrabold border border-rose-200 dark:border-rose-800">
                  🏃 Pulang Awal: <strong>{totalEarlyOutCount}</strong>
                </span>
              </div>
            </div>

            {/* Filter Control Inputs Bar */}
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-purple-950/50 border border-slate-200 dark:border-purple-900/60 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-2 text-xs rounded-xl border border-slate-300 dark:border-purple-800 bg-white dark:bg-purple-900/60 text-slate-800 dark:text-slate-100 font-semibold focus:outline-none focus:ring-2 focus:ring-amber-400"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300 dark:border-purple-800 bg-white dark:bg-purple-900/60 text-slate-800 dark:text-slate-100 font-semibold focus:outline-none focus:ring-2 focus:ring-amber-400 cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs rounded-xl border border-slate-300 dark:border-purple-800 bg-white dark:bg-purple-900/60 text-slate-800 dark:text-slate-100 font-semibold focus:outline-none focus:ring-2 focus:ring-amber-400"
                  />
                </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-200 dark:border-purple-900/40">
                <div className="flex items-center gap-1.5">
                  <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1">
                    <Filter className="w-3.5 h-3.5 text-amber-500" /> Preset:
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-2.5 py-1 rounded-lg bg-amber-400/20 text-purple-950 dark:text-amber-300 text-[10px] font-extrabold hover:bg-amber-400 hover:text-purple-950 transition-all cursor-pointer"
                  >
                    Hari Ini
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-rose-500/20 text-rose-600 dark:text-rose-400 text-[10px] font-extrabold hover:bg-rose-600 hover:text-white transition-all cursor-pointer"
                  >
                    Reset Filter
                  </button>
                </div>
                ...
                <div className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold">
                  Menampilkan <strong className="text-amber-600 dark:text-amber-300">{totalRecordsCount}</strong> dari {attendance.length} total rekaman
                </div>
              </div>
            </div>

            {/* Attendance Table */}
            <div className="rounded-xl border border-slate-200 dark:border-purple-900/50 overflow-hidden shadow-xs">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="bg-slate-100 dark:bg-purple-950 text-slate-700 dark:text-amber-300 font-extrabold border-b border-slate-200 dark:border-purple-900">
                      ...
                      <th className="p-3 text-center">Foto Swafoto</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-purple-900/40">
                    {filteredRecords.length === 0 ? (
                      ...
                      filteredRecords.map((rec) => (
                        <tr key={rec.id} className="hover:bg-slate-50 dark:hover:bg-purple-950/40 transition-colors">
                          <td className="p-3 align-top font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap">
                            <div className="font-mono text-xs">{rec.date}</div>
                            <div className="text-[10px] text-slate-500 font-semibold">{rec.clockInTime} WIB</div>
                          </td>

                          <td className="p-3 align-top font-bold text-slate-800 dark:text-slate-100">
                            <div className="text-sm font-extrabold text-[#3D1259] dark:text-amber-300">{rec.employeeName}</div>
                            <div className="text-[10px] text-slate-500 font-medium">🏪 {rec.outlet}</div>
                          </td>

                          <td className="p-3 align-top font-mono font-semibold text-slate-700 dark:text-slate-300">
                            <div>🟢 Masuk: <strong>{rec.clockInTime}</strong></div>
                            <div>🔴 Pulang: <strong>{rec.clockOutTime || 'Belum Pulang'}</strong></div>
                          </td>

                          <td className="p-3 align-top font-black text-amber-600 dark:text-amber-300 whitespace-nowrap">
                            {rec.hoursWorked ? `${rec.hoursWorked} Jam` : '-'}
                          </td>

                          <td className="p-3 align-top whitespace-nowrap">
                            <div className="space-y-1">
                              <span
                                className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold block w-fit ${
                                  rec.status === 'Hadir'
                          ...
                          <td className="p-3 align-top text-center">
                            {rec.selfieUrl ? (
                              <button
                                ...
                                className="relative group inline-block rounded-xl overflow-hidden border border-amber-400/50 shadow-xs cursor-pointer"
                          ...
                          <td className="p-3 align-top text-center">
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-rose-500/10 text-rose-600 hover:bg-rose-600 hover:text-white dark:text-rose-400 transition-all cursor-pointer"
                              title="Hapus Rekam Presensi Ini"
                            >
