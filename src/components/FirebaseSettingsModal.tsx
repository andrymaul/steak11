...
export const FirebaseSettingsModal: React.FC<FirebaseSettingsModalProps> = ({
  ...
  const handleSyncFirestore = async () => {
    setSaveMessage('Menyinkronkan data dengan Firebase Firestore...');
    await initializeFirestoreDatabase();
    setSaveMessage('Sinkronisasi data Firestore berhasil diselesaikan!');
    setTimeout(() => setSaveMessage(null), 3000);
  };
      ...
      >
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800 mb-6">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-br from-amber-500/20 to-purple-900/30 text-amber-500 rounded-2xl border border-amber-500/20">
              <Flame className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-slate-900 dark:text-white">Pengaturan Firebase Firestore</h2>
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black border ${
                  ...
                  {isConnected ? 'Firestore Connected' : 'Configuration Pending'}
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">Pusat kendali koneksi Database Realtime & Kredensial Firebase</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 transition-colors cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>

        {saveMessage && (
          <div className="mb-6 p-3.5 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200 rounded-2xl text-xs flex items-center gap-2 border border-emerald-200 dark:border-emerald-800 font-semibold shadow-xs">
            ...
            <div className="p-5 rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-500/5 via-purple-500/5 to-slate-900/5 dark:from-amber-950/20 dark:to-purple-950/20 shadow-xs">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-black border ${
                  ...
                  {isConnected ? 'ONLINE' : 'OFFLINE / FALLBACK'}
                </span>
              </div>
